...
type Props = {
  ...
  setProductorss: (productorss: TProductors) => { productorss?: TProductors };
};

const ProductorList = ({ productors, productorss, setProductorss }: Props) => {
  return (
    <>
  ...
  ({ database, search }: Props) => {
    ...
    return {
      productors: database
        ...
        .query(Q.where('productorname', Q.like(`%${query}%`)))
        .observe(),
    };
  }
);

export default withDatabase(withModels(ProductorList));
