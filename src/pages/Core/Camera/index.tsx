...
export const Camera: React.FC = () => {
  ...
  const validar = () => {
    ...
    if (nft_name === '') {
      setNft_nameError('registerError');
      error = true;
    }
    if (nft_price === '') {
      setNft_priceError('registerError');
      error = true;
    }
    if (nft_description === '') {
      setNft_descriptionError('registerError');
      error = true;
    }
    if (nft_chain === '') {
      setNft_chainError('registerError');
      error = true;
    }
    if (nft_chain_market === '') {
      setNft_chain_marketError('registerError');
      error = true;
    }
    if (nft_chain_url === '') {
      setNft_chain_urlError('registerError');
      error = true;
    }
    return !error;
  };
  ...
  async function testNetServer() {
    ...
    if (!isConnected) {
      onMessage('error.internet', '', 'danger');
    } else {
      returno = true;
    }
    return returno;
...
*/}
        ...
        {!isLoading && visibleButton && (
          ...
          >
            ...
            <Text style={styles.textDelete}> {'LECTOR DE CARAVANA'}</Text>
          </TouchableOpacity>
        )}
      </ScrollView>
