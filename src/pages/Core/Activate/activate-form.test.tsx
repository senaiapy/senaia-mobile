...
describe('ActivateForm Form ', () => {
  it('renders correctly', async () => {
    const { findByText } = customRender(<ActivateForm />);
  ...
  it('Should call ActivateForm with correct values when values are valid', async () => {
    const mockOnSubmit = jest.fn(({ email, password }) => {
      return Promise.resolve({ email, password });
    });

    const { getByTestId } = customRender(
      <ActivateForm onSubmit={mockOnSubmit} />
    );
