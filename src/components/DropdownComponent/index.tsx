...
import { useDetectDevice } from '@/components/utilsScale';
...
const ic_down = require('@/assets/icons/down.png');
    ...
    const _measure = () => {
      if (ref) {
        ref.current?.measure((width, height, px, py, fx, fy) => {
          ...
          const w = parseInt(px?.toString(), 10);
          const top = isFull
            ? 20
            : parseInt(py?.toString(), 10) + parseInt(fy?.toString(), 10) + 2;
          const bottom = H - top;
          const left = I18nManager.isRTL
            ? W - parseInt(px?.toString(), 10) - parseInt(fx?.toString(), 10)
            : parseInt(fx?.toString(), 10);
          ...
          setPosition({
            ...
            left,
            height: parseInt(py?.toString(), 10),
          });
        });
      }
    };
