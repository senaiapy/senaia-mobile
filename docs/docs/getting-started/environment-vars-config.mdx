...
import axios from 'axios';
export const client = axios.create({
  EXPO_PUBLIC_baseURL: Env.EXPO_PUBLIC_API_URL_FEED,
});
```
...
// default values
const SCHEME = 'com.pyfoundation.senaia';
const APP_NAME = 'Senaia';
...
const development = {
  ...
  EXPO_PUBLIC_API_URL_FEED: 'https://dummyjson.com/',
...
export type ConfigType = {
  ...
  EXPO_PUBLIC_API_URL_FEED: string;
  version: string;
  name: string;
};
...
import axios from 'axios';
export const client = axios.create({
  EXPO_PUBLIC_baseURL: Config.EXPO_PUBLIC_API_URL_FEED,
});
```
